...
**단일 프로세스를 전제합니다.** 미리보기 계획과 라이브러리 리비전 카운터는
프로세스 메모리에 있습니다. `run.sh`·Dockerfile·compose 어디에도 `--workers`가
없어 uvicorn 워커는 하나이므로 스레드가 이를 공유합니다. 워커를 늘리면 A에서 만든
미리보기를 B에서 실행할 수 없고, 계획 검증 시 서로의 쓰기를 보지 못합니다 —
수평 확장 전에 계획과 리비전을 SQLite로 옮겨야 합니다.
